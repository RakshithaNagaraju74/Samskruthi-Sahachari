...
export default function LeafletMap({ sites, userVisited = [], userBooked = [], onSiteClick }: HeritageMapProps) {
  ...
  // Add markers when map is ready or sites change
  useEffect(() => {
    if (!isMapReady || !mapRef.current || sites.length === 0) return;
    ...
    if (sites.length > 0) {
      const validSites = sites.filter(s => s.coordinates?.lat && s.coordinates?.lng);
      if (validSites.length > 0) {
        const bounds = L.latLngBounds(validSites.map(s => [s.coordinates.lat, s.coordinates.lng]));
