...
// Icons (same as user messages page)
const Icons = {
...
export default function EnterpriseMessagesPage() {
  ...
  const fetchConversations = async () => {
    setLoading(true);
    try {
      const data = await messageService.getEnterpriseConversations();
  ...
  const handleSendMessage = async () => {
    ...
    setSending(true);
    try {
      const sentMessage = await messageService.sendEnterpriseMessage(
              ...
              <>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  {messages.map((msg, index) => {
                    const isEnterprise = msg.sender_type === 'enterprise';
                    ...
                    return (
                      <div
                        key={msg.id}
                        className={`flex ${isEnterprise ? 'justify-end' : 'justify-start'}`}
                      >
                        <div className={`flex items-end gap-2 max-w-[70%] ${
                          isEnterprise ? 'flex-row-reverse' : 'flex-row'
                        }`}>
                          {!isEnterprise && showAvatar && (
                          ...
                          <div>
                            ...
                            <p className={`text-xs mt-1 ${isDarkMode ? "text-gray-500" : "text-gray-400"} ${
                              isEnterprise ? 'text-right' : 'text-left'
                            }`}>
                              {formatMessageTime(msg.created_at)}
                              {msg.is_read && isEnterprise && (
